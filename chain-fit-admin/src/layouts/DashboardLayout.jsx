...
import { Search, SlidersHorizontal, Sun, Moon, UserCircle } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

const DashboardLayout = () => {
    // Logika Dark Mode (sama seperti di Login)
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#fafafa] dark:bg-[#0a0a0a] text-gray-900 dark:text-gray-100 font-sans transition-colors duration-500">
            ...
            <header className="h-[72px] border-b border-gray-200 dark:border-white/10 bg-white/50 dark:bg-black/50 backdrop-blur-md sticky top-0 z-40 flex items-center justify-between px-8">
                ...
                {/* Tengah: Search Bar (Sesuai Desain) */}
                ...
                <div className="flex items-center gap-4">
                    ...
                    {/* Filter Button */}
                    <button className="hidden md:flex items-center gap-2 border border-gray-200 dark:border-white/10 bg-white dark:bg-[#111] px-4 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-[#1a1a1a] transition-colors text-[13px] font-medium text-gray-700 dark:text-gray-300">
                        All Status
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto p-8 overflow-x-hidden">
                {/* <Outlet /> akan me-render Dashboard.jsx di sini */}
                <Outlet />
            </main>
        </div>
    );
